...
在`src`文件夹中创建`command_parser.py`文件。
...
按照RESP规范，我们指定返回的数据类型前都需要加上一个字节的字符。
...
对于SET操作，实现就是引用我们先前创建的`memory`类，对其传入key和value，并在操作成功以后返回RESP规范要求返回的`OK`简单字符串。
...
```
对于GET操作，就是根据key从`memory`类中提取value，将其作为复杂字符串返回。
...
限于篇幅，本文档不会将所有命令处理函数贴上来，你可以在[源码](https://github.com/KeepitReal555/PyRedis/tree/Exp4)中查看它们。

#### 3.2.3 较特殊命令 EXPIRE/TTL

这两个命令跟其他命令不同，我们实现它需要借助第三方模块`gevent`。让我们来介绍下该模块：

#####  gevent

`gevent`是一个基于协程(Coroutine)的Python模块。

本实验使用了`gevent`的延时操作，我们使用`gevent.spawn_later()`方法来实现对数据的定时删除。

可以想象，一个进程正在向前运行，我们设置的期限就是壁垒，当进程到达这个壁垒时，便会自己结束自己。
...
本实验没有测试，等下一个实验完成PyRedis项目，我么就可以验证这些代码编写的正确性了。
